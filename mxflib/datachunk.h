...
namespace mxflib
{
	...
	public:
		...
		//! Set some const data into a data chunk (expanding it if required)
		void Set(size_t MemSize, const UInt8 Val, size_t Start = 0)
		{
			if(Size < (MemSize + Start)) Resize(MemSize + Start);

			memset(&Data[Start], Val, MemSize);
		}
		...
		//! Append some const data to a data chunk
		void Append(size_t MemSize, const UInt8 Val)
		{
			Set(MemSize, Val, Size);
		}
